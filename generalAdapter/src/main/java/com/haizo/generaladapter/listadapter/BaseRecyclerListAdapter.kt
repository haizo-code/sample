...
abstract class BaseRecyclerListAdapter internal constructor() : LoadMoreListAdapter() {
     ...
     *
     * - This method will return the container's adapter (in a Container ViewHolder)
     * - Your container ViewHolder should implement [ContainerViewHolder] so this method can work as expected
     */
    open fun getContainerAdapter(containerUniqueId: String): BaseRecyclerListAdapter? {
        val indexOfContainer = indexOf(containerUniqueId)
        return (recyclerView.findViewHolderForAdapterPosition(indexOfContainer) as? ContainerViewHolder)?.getInnerAdapter()
    }
    ...
    @Deprecated("Will be removed in next version, you should use getContainerAdapter() for mor flexible use")
